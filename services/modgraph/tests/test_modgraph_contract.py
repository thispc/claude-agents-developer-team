...
fails here before oasdiff ever sees a diff. It is also the fuzzer that found the
int64-overflow plan id: an id no row can ever have is an id that does not exist,
so it answers like one instead of raising three frames down.
"""
...
# filter_too_much: the bounded ids/limits legitimately reject a slice of the
...
def test_contract(case):
    # The conductor's client adds X-Service-Token on every call, so the contract is
