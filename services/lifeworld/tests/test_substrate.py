"""The substrate — ironclad, offline.
...
MOVED HERE BY THE P4 CUTOVER. Until then this lived in tests/test_lifeworld.py and
imported `app.lifeworld` directly. The substrate is another process now, and nothing
outside `services/lifeworld/` may import inside it (SERVICE_CONTRACT rule 9) — so the
tests that unit-test the engine came WITH the engine. What stayed in the conductor's
suite is the half that drives `/api/lw/*` over HTTP: that is a test of the doorway and
the seam, and it belongs where the doorway is.
"""
...
def _mod(name: str):
    """One substrate module, by name. `importlib` rather than `from substrate.x` at
    module scope because the package is registered by this suite's conftest during its
    own load — the import must happen after that, not at collection of this file."""
    return importlib.import_module(f"substrate.{name}")


World, Scene, Human, Card, Deck = _S.World, _S.Scene, _S.Human, _S.Card, _S.Deck
Flags = _mod("config").Flags
Signal, Packet = _mod("types").Signal, _mod("types").Packet
util, skills_mod, drives_mod = _mod("util"), _mod("skills"), _mod("drives")

LW = Path(__file__).resolve().parent.parent / "substrate"
...
def test_the_taxonomy_is_one_atom_two_kinds():
    from substrate.entity import Entity, Being
    from substrate.artifact import Artifact
...
def test_the_engine_imports_no_provider_except_the_one_door():
    """Only ports.py may reach for a provider — it IS the substrate's one door, and
    since P4 what is behind it is the conductor's model door rather than an import.

    store.py used to be the second exception, because its opt-in live path pulled that
    door open. It is not under substrate/ any more: it opens a database, which is the
    service's business and never was the engine's. So the exception list got shorter,
    which is the only direction it should ever move.
    """
    for f in LW.glob("*.py"):
        if f.name in ("ports.py", "__init__.py"):
            continue
        src = f.read_text()
...
def test_a_collating_artifact_remembers_its_shape_and_path():
    ...
    from substrate.artifact import Prop
    w = free_world()
...
def test_scene_rule_rows_are_validated_whitelisted_and_capped():
    from substrate.scene_rules import validate_rows, MAX_ROWS
    rows = validate_rows([
...
def test_a_clamp_rule_bounds_the_packet_and_leaves_other_fields_alone():
    from substrate.scene_rules import SceneRuleSet
...
def test_a_composite_deck_deals_a_card_sealed_to_only_its_holder():
    ...
    from substrate.components import LIBRARY
    w = free_world()
...
def test_protocol_defaults_are_backward_compatible_and_validated():
    ...
    from substrate.threads import protocol_of, clean_protocol, DEFAULT_PROTOCOL
...
def test_an_agent_consults_its_memory_before_thinking():
    ...
    """
    from substrate import human as hmod
...
# ---- the recalled-experience path (moved from tests/test_crew_loop.py by P4) ----

def test_recalled_experience_reaches_the_tier2_prompt():
