...
One program: a Python web server of about 27,000 lines doing five jobs and no
others.
...
An *API* is the list of requests a program will answer. There are 163; the
...
| The Atlas | `/api/graph/project/1` | One project drawn as its own map: the task DAG, who works each card, what it delivered |
...
Twenty-seven tables. Nearly all of them live in one file. The **module graph**'s six are not among them any more: with P5 the graph of modules the platform keeps about itself became its own service (`services/modgraph`, its own process, its own `data/modgraph.db`), and `graph_plans`, `graph_nodes`, `graph_edges`, `graph_node_runs`, `graph_node_tests` and `graph_assign` went with it — along with the three things computed FROM them, because a derivation performed on a copy of a table you no longer own can disagree with the table. The screen did not move an inch: `/api/graph/*` is still the conductor's, and it composes the service's rows with facts only the conductor has (who an agent id is, what the crew is touching right now). What deliberately did NOT move is the part that thinks: the crew's manager authoring the plan needs a model, the crew's roster and the repair budget, so it stayed, and the store — which has never been allowed near a model — became the thing it writes into. That is also what broke the one real circular dependency in the whole decomposition. The trace (`graph_node_runs`) is the row worth naming: mastery of a module is COUNTED from it and never stored, so the conductor dropped its copy only after the service confirmed it had taken them — dropping early would have quietly un-mastered every module and let the next plan reshuffle specialists off work they had earned. The **knowledge** table is not among them either: what agents have learned lives with the knowledge service (`services/knowledge`, its own process, its own `data/knowledge.db`) — it carries embedding vectors, is the one store queried by similarity rather than by key, and the conductor reaches it over HTTP through a shim that degrades to empty answers rather than ever blocking a sprint. Two more stores left in P2 and the count did not move, because neither had ever been a table: the **quota meter** and the **notifier's memory** were kv blobs, each rewritten whole on every write — a lost update waiting for a second process — and they are now proper tables owned by their own services (`services/usage`'s `usage_rows` in `data/usage.db`; `services/notify`'s `notify_seen` and `notify_sent` in `data/notify.db`). P3 did the same to the largest blob of the lot and again the count did not move: the **log ring**, the **error ring** and the monitor's **decisions** were three kv values — the first rewritten whole on every single line the backend wrote — and they are now `log_rows`, `error_rows` and `decisions` in `services/watch`'s own `data/watch.db`. P4 took a real table out and the count moved for the second time: the **lifeworld** — the society of agents behind the Studio canvas and the self-repair crew — is its own service now (`services/lifeworld`, its own process, its own `data/lifeworld.db`), and `lw_worlds` went with it. A whole world is still one JSON row, because a world is read and written whole per scan as a playground rather than queried column by column; that row is simply another process's. The copy kept the row IDS, which is the only part that could not be got wrong: every world id on the platform is a pointer at one — the crew's own world, a staffing room, a project's team — so a renumbering would have un-staffed the crew and pointed every project's team at nothing. The conductor dropped its copy only after the service confirmed it had taken them. The ones worth knowing: **projects** and **tasks**
