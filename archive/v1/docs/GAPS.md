...
| G3 | ✓ | **Git host is configurable; the dashboard's links are not.** `GIT_API`/`GIT_WEB`/`GIT_PROVIDER` route every API call, clone and credential check, and Gitea's real divergences (merge verb and body key, no `head` filter on list-pulls, `limit` not `per_page`, paged trees) are handled. Still hardcoded: the `github.com` links built in `routes.py` and `dashboard/js/projects.js` and `dashboard/js/core.js`, and the k8s launcher does not pass `GIT_WEB` into the worker's env, so k8s workers still clone from github.com. |
...
| G5 | ◐ | **Any OpenAI-compatible endpoint is now bring-your-own inference.** A user configures a base URL, an optional key and a model list; `providers.py` drives it through `POST {base}/chat/completions`, which is the one wire format vLLM, Ollama, LiteLLM, Azure OpenAI and most gateways all speak. Endpoints are per-user in settings on the same root-only inheritance rule as the keys, and verify through the same Settings button (`credcheck.check_endpoint` lists the server's models, then makes one real completion — a model id the server does not have is the commonest fault and is named). Azure works because the deployment URL's query string is preserved and the key can go under `api-key`. **Bedrock and Vertex are configurable, not supported**: Bedrock's native runtime needs SigV4 signing and Vertex needs a service-account OAuth2 token that expires hourly, and neither is a string that can be pasted into a form and keep working. Both are reachable only through something that already terminates that auth — Bedrock's own OpenAI-compatible endpoint with a bearer key, or a proxy the user runs. Untested against either. |
...
| G30 | ◐ | **Notes attach to a task, a sprint or the project, and reach the manager with that object's state.** `feedback.py` records the note against its target and renders it into a directive — the channel that already works — carrying the task's number, title, status, PR and whether anything verified it, or the sprint's counts. Recording and delivering are separate: a note on a `done`/`failed`/`cancelled` project is held rather than queued, because a directive nobody consumes is indistinguishable from one ignored, and it is flushed on restart. Late feedback is delivered, not expired, but labelled with how many sprints old the work is and told that rework belongs in the current cycle — otherwise the manager either rewrites a frozen sprint or silently drops the note. No dashboard surface yet: `POST/GET /api/projects/{id}/feedback`, `POST /api/feedback/{id}/resolve`. |
