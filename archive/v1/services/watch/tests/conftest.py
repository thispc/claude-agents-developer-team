"""Test env for the watch service, set BEFORE app/helpers latch their config.

Offline by construction: a temp database, a known token, no legacy db, no
conductor and no sockets — the suite drives the ASGI app in-process.
...
run INSIDE the full repo run (pytest.ini's testpaths include services/):
  ...
  1. The service is loaded under UNIQUE module names ("watch_service_app",
     "watch_service_helpers") via importlib, because the conductor's `app`
     ...
     modules with a plain `import watch_service_app`, which resolves from
...
_TMP = tempfile.mkdtemp(prefix="watch-svc-test-")
...
os.environ["DB_PATH"] = str(Path(_TMP) / "watch-test.db")
os.environ["SERVICE_TOKEN"] = TOKEN
os.environ["SERVICE_NAME"] = "watch"
...
@pytest.fixture()
def clean_store():
    """An emptied watch for one test: both rings, the decisions and the standing
    one. Leaving `auto` behind is how a suite silently starts approving."""
    con = svc.helpers.db()
    for table in ("log_rows", "error_rows", "decisions"):
        con.execute(f"DELETE FROM {table}")
    con.execute("DELETE FROM kv WHERE key = 'auto'")
    con.commit()
    svc.MAX_ROWS = 3000
    svc.MAX_ERRORS = 300
    yield svc
