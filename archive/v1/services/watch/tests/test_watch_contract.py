...
import watch_service_app as svc                         # noqa: E402  (see conftest)

SPEC = Path(__file__).resolve().parent.parent / "openapi.json"

# The ring caps would otherwise be reached by the generator and the later cases
# would all exercise one branch (everything trimmed). Raised at module scope
# rather than per case, because a contract run generates hundreds of calls.
svc.MAX_ROWS = 10 ** 6
svc.MAX_ERRORS = 10 ** 6
...
# filter_too_much: the bounded windows/limits legitimately reject a slice of the
