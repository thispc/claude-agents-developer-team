...
The P3 cutover finished here: the vendored in-process body (_logs_legacy.py) and the
dual-mode branch are gone, and `WATCH_URL` is now REQUIRED. It is written into
data/env/conductor.env by tools/gen_fleet.py from services.yaml, so every supported boot
path — `./run-local.sh` and `./run-local.sh --legacy` alike — has it. A conductor started by
hand without it fails loudly in init() rather than pretending to keep a record.

WHY init() AND NOT IMPORT. Importing this module must stay free: tools, tests and the module
graph import it without ever writing a row, and most of the conductor imports it
transitively. Boot does call init(), and that is the honest place to refuse — one clear
message naming run-local.sh, before the first thing that goes wrong goes unrecorded.
...
_NO_URL = (
    "WATCH_URL is not set — the log ring is a service since P3 and the conductor has no "
    "in-process fallback any more. Boot with ./run-local.sh (or ./run-local.sh --legacy), "
    "which generates data/env/conductor.env from services.yaml and starts services/watch "
    "on 8884."
)

# The four keys the strangler left behind in devteam.db. Named here, in the module that used
# to write the first two, because this is where all four are dropped.
_MIGRATED_KEYS = ("logs:ring", "logs:errors", "monitor:decisions", "monitor:auto")


def init() -> None:
    """Refuse to boot without the ring, and finish the strangler.

    The conductor keeps no log state at all any more. Four kv keys go — but only once the
    service says it has finished with them, which is a condition and not a formality:

      logs:ring         the pre-P3 ring, rewritten whole on every line, and its longer-lived
      logs:errors       error twin. The service copied both into `log_rows`/`error_rows` on
                        its own first boot (read-only ATTACH, marked once). Commit A kept
                        them so a rollback — unset the URL — could find the record again;
                        with the fallback deleted they are a second, staler copy of rows
                        another process owns, and one that no longer grows.
      monitor:decisions what the owner already dismissed or approved. THIS is the key that
                        makes the drop conditional. Deleting it before the service has copied
                        it does not lose data the platform can shrug at — it loses every
                        answer the owner has ever given, and the next scan asks all of them
                        again at once. That storm is the exact failure the copy exists to
                        prevent; re-creating it here would be perverse.
      monitor:auto      the standing permission for unattended approval. Not data — a
                        SETTING, and one whose absence silently changes behaviour: losing it
                        turns unattended approval off without anyone deciding that. The safe
                        direction, which is precisely why it would go unnoticed.

    NOTHING ORDERS THE TWO PROCESSES. process-compose starts the conductor and the watch
    service together, with no depends_on between them, so on the first boot after the cutover
    this can easily run before the service has attached. `GET /health` answers `backfilled`
    for exactly that: it means "the first-boot decision has been made, either way". Until it
    is true the keys stay and the next boot tries again — a dead key costs nothing, and
    nothing in the conductor reads any of the four ever again.
    """
    if not _URL:
    ...
    try:
        ...
        # this answer is four keys nobody reads.
        _note(f"cannot ask the watch service whether it has copied the old log keys yet — "
              f"leaving them in place ({type(e).__name__}: {str(e)[:120]})")
        return
    if not settled:
        return
