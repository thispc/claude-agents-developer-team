"""The pre-P3 in-process log pipeline, vendored for the strangler window.

This file exists only between commit A (extract) and commit B (cutover) of the
watch extraction: conductor/app/logs.py is the dual-mode shim, and when WATCH_URL
is unset it hands the whole surface to this module — byte-for-byte the old
behaviour, including the `logs:ring` and `logs:errors` kv blobs in devteam.db.
Commit B deletes this file and the fallback branch with it. Nothing imports this
module except the shim's fallback branch (and _monitor_legacy.py, which is the
other half of the same rollback); the moved-out body now lives, adapted to real
capped tables, in services/watch/app.py.

The one thing to know while reading it: every `log()` below is a read-append-write
of one kv blob under a THREAD lock, on the platform's hottest path — 64 call
sites, some inside exception handlers. That lock stops two threads from losing a
line and does nothing at all about two processes, which is why the ring left the
conductor.
...
#
# logs_routes.py has ONE shape and both modes have to satisfy it. The client
# gained two names the in-process ring has no use for — a read either reaches the
# ring or it does not, and in this mode it always does. Answering honestly here
# is what lets the route stay a single code path. Commit B deletes this whole
# file, and these with it.

def degraded() -> bool:
    """The ring is this process. There is nothing to be unable to reach."""
    return False
...
def flush() -> int:
    """Nothing is queued: a write here IS the store. Present so the monitor's
    read-your-own-writes call is a no-op rather than an AttributeError."""
    return 0
