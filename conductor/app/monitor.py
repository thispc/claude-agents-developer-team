...
DUAL-MODE, for the strangler window between commit A and commit B: WATCH_URL unset hands the
whole surface to _monitor_legacy.py — byte-identical pre-P3 behaviour, including the
`monitor:decisions` and `monitor:auto` kv keys and the cross-owner read. Commit B deletes it
and those keys with it.
...
if not _URL:
    from . import _monitor_legacy as _legacy
