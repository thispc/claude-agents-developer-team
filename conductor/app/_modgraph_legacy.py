"""_modgraph_legacy.py — the PRE-P5 in-process module graph store, vendored unchanged.

THIS FILE IS THE ROLLBACK, and it exists only between commit A and commit B.
`modgraph.py` installs it wholesale in `sys.modules` when `MODGRAPH_URL` is
unset, so a fallback boot is byte-identical to the platform before the
extraction: same functions, same six tables in devteam.db, same source under
`inspect.getsource`. Commit B deletes it, and the rollback becomes `git revert`.

The one thing P5-A deliberately did NOT do is rename these tables aside. P1-P3
renamed theirs, because their rollback read a table the shim had moved. Here the
rollback is THIS file, which reads `graph_plans` and its five siblings BY NAME —
so a rename would make a rollback find six empty tables, reseed a fresh plan, and
lose every closed run the crew had earned mastery from. The rows stay put; the
service copies them out with the ids preserved (`services/modgraph/store.py`);
commit B drops them, and only once the service says it has them.

Everything below this line is the module as it stood before the extraction.
...
# --- the P5-A adapter block: three names the CALLERS gained ------------------
#
# Everything above this line is the module as it stood before the extraction.
# These three are the small price of a dual-mode window, and they are here for
# one reason: `routes/graph.py` and `modgraph_author.py` are shared by both
# modes, and P5 taught them to ask for a bulk read, a whole-plan write and an
# honest "could I see the store". Without these, unsetting MODGRAPH_URL would
# 500 the very screen the rollback exists to preserve — the same lesson P3-A
# learned with the log shim. Commit B deletes this file and the block with it.

def degraded() -> bool:
    """In-process there is nothing to be unable to reach. The BFF asks this to
    tell "the graph is empty" from "the graph could not be read", and here the
    second case cannot happen."""
    return False


def assigns(plan_id: int) -> dict:
    """Every assignment on a plan, by node key. One query rather than one per
    node — the shape the payload and the engine's matcher actually want, and the
    reason it exists at all is that over a wire the old shape was a round trip
    per node."""
    return {r["node_key"]: r for r in
            db._rows("SELECT * FROM graph_assign WHERE plan_id=?", (int(plan_id),))}


def import_plan(project_id: int, *, kind: str = "template", authored_by: str = "manager",
                notes: str = "", nodes: list[dict], edges: list[dict],
                tests: list[dict], assigns: dict | None = None,
                positions: dict | None = None, activate: bool = True) -> dict | None:
    """A whole plan version in one call — the same rows the two bulk writers used
    to write inline, in the same order. In-process there is no transaction to
    win: every helper commits its own statement, exactly as it always did."""
...
# be earnable within a day of sprints. (Since P5 the live copy is the service's, in
# services/modgraph/derive.py; this one is here because the rollback has to count
# somewhere, and the somewhere is the process holding the rows.)
MASTER_RUNS = 3
...
def mastery(project_id: int = 0) -> dict[str, dict]:
    ...
    construction. Ties keep the incumbent — a challenger catches up to a master, it
    does not split the title, and a later arrival must EXCEED the master's count."""
    rows = db._rows(
