...
`mastery` and the seed all moved. This file is the client, keeping every public
name the in-process module ever had — `create_plan`, `nodes`, `edges`,
`note_run`, `close_run`, `map_test`, `set_assign`, `positions`,
...
DUAL-MODE, for the strangler window between commit A and commit B:
  ...
  MODGRAPH_URL set    → the HTTP client below. gen_fleet writes the URL into
                        data/env/conductor.env from services.yaml, so a fleet
                        boot (./run-local.sh) is in this mode by construction.
  MODGRAPH_URL unset  → the old in-process body, vendored unchanged in
                        _modgraph_legacy.py, INSTALLED WHOLESALE: this module
                        replaces itself in sys.modules with the legacy module,
                        so fallback mode is byte-identical to pre-P5 — same
                        functions, same six tables in devteam.db, same
                        introspectable source. (`./run-local.sh --legacy` runs
                        here in commit A.) Commit B deletes the legacy module
                        and this branch.

The mode is decided at import: a process boots into one world and stays there.

LATENCY BUDGET (URL mode): one localhost round trip per verb, ~1-3ms; hard
timeout 2s so a wedged service can never hold a sprint or a poll. The Atlas's
single read (`/api/graph/self`) costs SEVEN of them — plan, nodes, edges, tests,
assigns, runs, positions — plus one for mastery, which is why `assigns()` exists
at all: per-node `get_assign` would have made it twenty-one on a payload that is
polled, and a faithful port that triples the latency of the screen is not
faithful.
...
if not _URL:
    ...
    # after executing this file, so `from . import modgraph` everywhere yields
    # the legacy module itself — its functions, constants and source, unchanged.
    from . import _modgraph_legacy as _legacy
    ...
    _TIMEOUT = 2.0
    # A plan import writes a whole version in one transaction, and the seed builds
    # its manifest by reading ~20 source files and parsing every test module. Both
    # are well under a second in practice; 15s is the "something is wrong" line,
    # not the expected cost.
    _SLOW_TIMEOUT = 15.0

    # Tests inject an httpx transport here (ASGITransport onto the mounted
    # service, or one that raises) — the client code path stays identical.
    _TRANSPORT: httpx.BaseTransport | None = None
    _TOKEN = ""

    # Whether the LAST read reached the service. An empty graph and a graph that
    # could not be read look identical on screen, and the difference is the whole
    # question ("this repo has no plan" vs "you cannot see the plan") — so the BFF
    # carries this into the payload instead of leaving the Atlas to guess. Set by
    # the read verbs; costs no extra round trip. Same seam as logs._degraded_read.
    ...
    def _client(timeout: float = _TIMEOUT) -> httpx.Client:
        ...
        engine's build hooks, the authoring pass's row writes — is a plain
        function on a path that has never been awaitable, and making twenty-five
        verbs async would ripple through every route and every test that drives
        them for no gain a localhost round trip can measure. A blocked event loop
        is bounded by the timeout; the same trade usage.py documents for the meter.
        Tests swap this factory for a TestClient on the mounted service."""
    ...
    def _degraded(verb: str, err: Exception) -> None:
        ...
        and it is also the TRACE GAP being recorded as a gap."""
        try:
            from . import logs
    ...
    def _read(verb: str, path: str, key: str, empty, params: dict | None = None,
        ...
        """One read, its empty shape, and the honest flag. Every read verb goes
        through here so none of them can forget to say it could not see."""
    ...
    def degraded() -> bool:
        """Did the last read fail to reach the store? The BFF puts this on the
        payload so the Atlas can say "graph unavailable" instead of drawing an
        empty repository, and so a picker never reads an outage as a fact."""
    ...
    def note_run(plan_id: int, node_key: str, kind: str, *, task_id: int | None = None,
        ...
        ZERO IS THE DEGRADED SHAPE AND IT IS LOAD-BEARING. The engine stores what
        this returns and closes it later; a fabricated id would close somebody
        else's row when the service came back. Falsy means "there is no run to
        close", which the build hook already handles because a task that matched
        no node has always meant the same thing."""
        try:
    ...
    def set_assign(plan_id: int, node_key: str, *, agent_id: int | None = None,
        ...
        # The historical signature never returned None, and its one caller reads
        # two keys straight off the answer. Echo what was asked for rather than
        # inventing what is stored — nothing was stored.
    ...
    def assigns(plan_id: int) -> dict[str, dict]:
        """Every assignment on the plan, by node key — one call for the callers
        that want all of them (the payload, and the engine's node matcher)."""
    ...
    def derive_group_edges(child_edges: list[dict], parent_of: dict[str, str],
        ...
        """The top tier, reconciled against an authored one — computed by the
        service because it is GRAPH DOMAIN, not presentation: the seed calls it to
        build the plan it writes, and the payload calls it again to reconcile what
        a manager authored. Two copies of that rule on two sides of a wire is how
        the stored plan and the rendered graph start telling different stories.

        Degraded → [], so the top room shows its cards with no arrows between
        layers rather than arrows nothing backs."""
        try:
    ...
    def mastery(project_id: int = 0) -> dict | None:
        """{node_key: {agent_id, runs, master}}, or None when the store cannot be
        read. NONE, NOT {} — the authoring pass's "a master keeps its module"
        rule reads this, and an outage that looked like "nobody has earned
        anything" would let one reshuffle undo every earned continuity on the box.
        The payload treats None as "no mastery to show", which is honest."""
        global _degraded_read
        try:
    ...
    def seed_self_graph() -> int:
        ...
        Degraded → 0 with a log, and boot carries on: main.py already guards this
        call because a seed failure must never block startup, and the graph screen
    ...
    def self_manifest() -> dict:
        """The repo's real module inventory, read from the working tree by the
        service — the ONE builder, shared by the offline seed and the manager's
        authoring pass, so both describe the same tree.
    ...
    def _tests_for_nodes(by_key: dict[str, list[str]]) -> dict[str, set[str]]:
        """Which test files exercise which AUTHORED boundary. Sets are not JSON,
        so the wire carries sorted lists and this end restores the shape its one
        caller (the authoring pass's candidate manifest) has always used."""
        global _degraded_read
        try:
    ...
    def health() -> bool:
        ...
        around it, so every verb and any future card agree on what "up" means."""
        try:
    ...
    def init() -> None:
        """Commit A: the six tables stay exactly where they are.

        They are NOT renamed aside — a departure from P1-P3 and the same call P4
        made. The rollback here is `_modgraph_legacy.py`, which reads
        `graph_plans` and its five siblings by name; renaming them would make a
        rollback find six empty tables, reseed a fresh plan and lose every closed
        run the crew earned its mastery from. The service copies them out over a
        read-only ATTACH with the ids preserved, and commit B drops them — ONLY
        once `/health` says `backfilled`, because nothing orders the two
        processes and a premature drop takes the trace with it."""
        return None
