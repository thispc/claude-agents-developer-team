"""The pre-P1 in-process knowledge body, vendored for the strangler window.

This file exists only between commit A (extract) and commit B (cutover) of the
knowledge extraction: conductor/app/knowledge.py is the dual-mode shim, and when
KNOWLEDGE_URL is unset it hands the whole surface to this module — byte-for-byte
the old behaviour, including its own `knowledge` table in devteam.db. Commit B
deletes this file and the fallback branch with it. Nothing imports this module
except the shim's fallback branch; the moved-out body now lives, adapted to its
own store, in services/knowledge/app.py.

What an agent has learned, stored so it can be found again — a generic black box
behind four verbs:
...
def init() -> None:
    # Strangler window (P1, between commits A and B): a URL-mode boot renames
    # knowledge → knowledge_legacy so the service's first-boot backfill can copy
    # the rows and the conductor stops owning the table. Rolling BACK to this
    # in-process mode (KNOWLEDGE_URL unset again) must return those rows, not
    # start an empty store — so undo the rename when it happened and nothing has
    # re-created a knowledge table since.
    names = {r["name"] for r in db._rows(
        "SELECT name FROM sqlite_master WHERE type='table'"
        " AND name IN ('knowledge','knowledge_legacy')")}
    if "knowledge_legacy" in names and "knowledge" not in names:
        db._execute("ALTER TABLE knowledge_legacy RENAME TO knowledge")
